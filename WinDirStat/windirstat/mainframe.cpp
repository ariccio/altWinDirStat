...
void CMySplitterWnd::OnSize(UINT nType, const int cx, const int cy)
{
	if (GetColumnCount() > 1)
	{
...
void CMainFrame::MyQueryRecycleBin(CRecycleBinApi& rb, LONGLONG& items, LONGLONG& bytes)
{
	...
	// Why ever pass us non-zero values??!?
	//ASSERT( items == 0 );
	//ASSERT( bytes == 0 );
	...
	for (i=0; i < 32; i++, mask <<= 1)
	{
		...
		SecureZeroMemory( &qbi, sizeof( qbi ) );
...
void CMainFrame::OnSize(UINT nType, const int cx, const int cy)
{
