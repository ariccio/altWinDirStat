...
//_Pre_satisfies_( ThisCItem->m_type == IT_DIRECTORY )
//_Pre_satisfies_( ThisCItem->m_children != NULL ) 
std::pair< std::vector<std::pair<CItemBranch*, std::future<std::uint64_t>>>, std::vector<std::future<std::vector<std::pair<CItemBranch*, std::future<std::uint64_t>>>>>> DoSomeWork                    ( _In_ CItemBranch* const ThisCItem, std::wstring path, _In_ const CDirstatApp* app, const bool isRootRecurse = false );
